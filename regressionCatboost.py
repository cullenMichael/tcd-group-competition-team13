...
def main():
    ...
    print ('Fitting')
    parameters = {'depth'         : sp_randInt(7,14),
                  'learning_rate' : sp_randFloat(),
                  'iterations'    : sp_randInt(1000, 1500)
                  }

    randm = RandomizedSearchCV(estimator=regressor, param_distributions=parameters,
                               cv=4, n_iter=10, n_jobs=6)
    randm.fit(X_train1, y_train)
    ...
    itir = randm.best_params_['iterations'] + 500
    regressor = CatBoostRegressor(iterations=itir,
    ...
    _, predict_X = train.align(predict_X, join='outer', axis=1, fill_value=0)

    print ('Fitting Test Data')
    regressor.fit(X_train1, y_train, eval_set=eval_dataset)
